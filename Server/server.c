...
int main()
{
    // 🛠️ [STABILITY] تهيئة دوال الحماية والـ Signals (زي الـ Ctrl+C)
    setup_stability();
    ...
    for(int i = 0; i < MAX_CLIENTS; i++)
    {  
        reset_player(&clients[i]); // 🛠️ [STABILITY] تصفير آمن لبيانات ومؤشرات اللاعبين
    }
    ...
    for(;;)
    {  
        // 🛠️ [STABILITY] الخروج الآمن وتنظيف السيرفر عند الضغط على Ctrl+C
        if (got_sigint) 
        {
        ...
        clean_zombie_processes(); // 🛠️ [STABILITY] تنظيف العمليات الميتة (Zombies) لمنع استهلاك ميموري الـ OS
        watchdog_check(&fsm);     // 🛠️ [STABILITY] مراقبة حالات السيرفر لحل أي تعليق أو Freeze فوري
        ...
        {
            int conn_sock = Accept(listen_sock, NULL, NULL);
            set_nonblocking(conn_sock); // 🛠️ [STABILITY] جعل السوكيت Non-blocking لحماية السيرفر من الـ Blocking Attacks
            printf("New player connected! FD: %d\n", conn_sock);
        ...
        for(int i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            {    
                Message msg;
                // 🛠️ [STABILITY] استبدال الـ recv العادية بدالتك الآمنة لفحص الـ Flood والـ Validation والـ Overflows
                ...
                if (msg.type == ANSWER) {
                    // 🛠️ [STABILITY] تعديل الشرط ليشمل استقبال الإجابات العادية أو الـ Replay بأمان دون تعليق السيرفر
                    if ((fsm.current == STATE_WAIT_FOR_ANSWERS || fsm.current == STATE_WAIT_REPLAY) && !clients[i].hasAnswered) {
                        clients[i].hasAnswered = true;
                        clients[i].lastAnswer = atoi(msg.data);
                        gettimeofday(&clients[i].responseTime, NULL);
        ...
        {
            case STATE_LOBBY:
                // 🛠️ [STABILITY] استخدام تايمر خلفي غير معطل (Non-blocking) بديل لـ sleep القديمة التي كانت توقف السيرفر كاملاً
            ...
            case STATE_WAIT_FOR_ANSWERS:
                long current_time = get_current_time_ms();
                // 🛠️ [STABILITY] إصلاح وتأمين الـ Logical Operator (إضافة علامة ||) لمنع الـ Syntax & Compile error
                if ((current_time - question_start_time_ms >= QUESTION_TIMEOUT * 1000) || 
                    (fsm.answersReceived >= fsm.activePlayers && fsm.activePlayers > 0))
                {
                    fsm.current = STATE_CALCULATE_RESULTS;
